...
# Run: curl https://raw.githubusercontent.com/Surveily/Images/master/script/k3s-multi-init.sh | sudo sh
...
# Install K3S
curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="--no-deploy traefik" INSTALL_K3S_VERSION="v1.21.7+k3s1" sh -s - server --cluster-init
systemctl stop k3s
